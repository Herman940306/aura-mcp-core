...
CHAT_TIMEOUT_S = _env_float("AURA_CHAT_TIMEOUT", 90.0)  # 90s for CPU inference
# Default max tokens - keep small for fast responses
CHAT_MAX_TOKENS = _env_int("AURA_CHAT_MAX_TOKENS", 64)
...
class MCPToolRegistry:
    ...
    def __init__(self, backend_url: str = "http://localhost:9201"):
...
```"""
    ...
    def _get_ollama_backend(self):
        ...
        if not hasattr(self, "_ollama_backend"):
            ...
            # Use port 9207 as specified in PRD Section 8.13
            self._ollama_backend = OllamaBackend(
                base_url="http://localhost:9207",
                model="llama3",
            )
...
Type 'list tools' to see all {tool_count} available tools."""
    ...
    async def _handle_mcp_request(
        ...
        """Central MCP router: deterministic, no LLM involvement."""
        msg = message.lower()
        ...
        if any(tool_list_patterns):
            ...
            # Ollama-specific queries - list models or get info
            ...
            payload = {
                "response": "MCP request understood, but no registered MCP operation exists. Try asking 'what can you do?' or 'help' to see available capabilities.",
                "llm_used": False,
                "source": "mcp_router",
            }
    ...
    def _get_llm(self):
        """Lazy load the DualModelAdapter (TALKER + WORKER)."""
        if self._llm is None:
            try:
                ...
                self._llm = DualModelAdapter.get_instance()

                # Register tools with the LLM
                for tool in self.tool_registry.get_tool_schemas():
                    # Note: actual execution goes through tool_registry, not LLM
                    pass

                self._llm_available = self._llm.is_model_available()
                print(
                    f"🧠 DualModelAdapter loaded: {self._llm.get_model_info()}"
                )
            except Exception as e:
                print(f"⚠️ DualModelAdapter not available: {e}")
    ...
    async def _run_llm_chat(
        ...
        Uses DualModelAdapter: TALKER (Phi-3) for fast responses, WORKER (Qwen) for reasoning.
        """
        ...
        try:
            ...
            print(
                f"ℹ️ llm.chat completed in {duration_ms}ms (tokens={max_tokens}, model={model_used}:{model_name})"
            )
            if duration_ms > int(CHAT_WATCHDOG_S * 1000):
                self._llm_hang_ts = time.time()
                self._llm_hang_reason = f"llm.chat exceeded watchdog {CHAT_WATCHDOG_S}s (actual {duration_ms/1000:.2f}s)"
            return result, None
        except TimeoutError:
            duration_ms = int((time.time() - start) * 1000)
            print(f"⚠️ llm.chat timed out after {duration_ms}ms")
            self._llm_hang_ts = time.time()
            self._llm_hang_reason = f"llm.chat exceeded timeout {timeout_s}s (watchdog {CHAT_WATCHDOG_S}s)"
            return None, "timeout"
        except Exception as e:  # noqa: BLE001
            duration_ms = int((time.time() - start) * 1000)
            print(f"❌ llm.chat failed after {duration_ms}ms: {e}")
            return None, str(e)
        finally:
        ...
        Returns:
            {
                ...
                "conversation_id": str,
                "mode": str,
                "model_used": str (talker|worker),
            }
        """
        conv = self.get_or_create_conversation(conversation_id, mode)
        conv.mode = mode  # Allow mode changes

        # Add user message
        conv.add_message("user", message)

        # MCP-first routing: detect MCP/system intents and handle via MCP services deterministically.
        if self._is_mcp_intent(message):
            return await self._handle_mcp_request(
                message, conversation_id, mode
            )

        llm = self._get_llm()
        tool_calls = []
        model_used = "talker"  # Default

        # Determine if WORKER model needed (heavy reasoning)
        force_worker = self._requires_worker(message)

        # Resolve runtime defaults from adapter (honors env overrides), with safe fallbacks.
        llm_temp = (
            getattr(llm, "default_temperature", 0.2)
            if hasattr(llm, "default_temperature")
            else 0.2
        )
        llm_top_p = (
            getattr(llm, "default_top_p", 1.0)
            if hasattr(llm, "default_top_p")
            else 1.0
        )
        llm_top_k = (
            getattr(llm, "default_top_k", 40)
            if hasattr(llm, "default_top_k")
            else 40
        )
        llm_rp = (
            getattr(llm, "default_repeat_penalty", 1.1)
            if hasattr(llm, "default_repeat_penalty")
            else 1.1
        )
        max_tokens = CHAT_MAX_TOKENS

        if not self._llm_available:
            # Fallback: Use simple pattern matching and tools without LLM
            response, tool_calls = await self._fallback_response(message, mode)
        else:
            # Use the DualModelAdapter (TALKER or WORKER based on message complexity)
            try:
                llm.load_model("worker" if force_worker else "talker")
                ...
                if err:
                    ...
                    model_used = result.get("model_used", "talker")
                ...
                if result and result.get("tool_call"):
                    ...
                    follow_up, err2 = await self._run_llm_chat(
                        llm,
                        messages=conv.get_messages_for_llm()
                        + [
                            {
                                "role": "user",
                                "content": "Please summarize the tool result for me.",
                            }
                        ],
                        mode=mode,
                        max_tokens=max_tokens,
                    ...
                    if err2:
                        response = (
                            "Tool executed, but follow-up LLM reply timed out."
                        )
                    else:
                        response = follow_up["response"]

            except Exception as e:
                response = f"I encountered an error: {str(e)}. Let me try using the tools directly."
                response, tool_calls = await self._fallback_response(
                    message, mode
                )
...
def get_chat_service(
    backend_url: str = "http://localhost:9201",
    auto_load_model: bool = True,
) -> ChatService:
    """Get the singleton chat service instance.

    Args:
        backend_url: URL of the MCP backend server
        auto_load_model: If True, automatically load the LLM on first chat
    """
    global _chat_service
    if _chat_service is None:
        _chat_service = ChatService(backend_url=backend_url)
